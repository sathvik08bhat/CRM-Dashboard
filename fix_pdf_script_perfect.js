...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    T('Andheri - Kurla Road, Andheri (E), Mumbai 400059', ML, 55);
    T('Tel: 91 22 42959123, Mail: info@kenmccoy.in, Web: www.kenmccoy.in', ML, 68);
    ...
    const chgH = Math.max(18, doc.heightOfString(chgText, {width: sw[1]-10, font: 'Helvetica-Bold', size: 9.5}) + 10);
    ...
    const dR_T=(l,v,b, dts=true)=>{
        if(dts) { doc.dash(1, {space: 1.5}).strokeColor('#000000').lineWidth(0.5); }
        else { doc.undash().strokeColor('#000000').lineWidth(0.5); }
        
        doc.moveTo(gL,y).lineTo(MR,y).stroke();
        doc.moveTo(gVL,y).lineTo(gVL,y+20).stroke();
        doc.undash();
    ...
    dR_T('Net Payable',invoice.netPayable,true, false); // bottom solid
    ...
    console.log("Successfully replaced the PDF Generation block and merged the two tables with zero space!");
} else {
