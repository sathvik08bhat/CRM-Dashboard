...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    doc.fontSize(22).fillColor('white').font('Helvetica-Bold'); T('Ken McCoy Consulting', ML, 18);
    doc.fontSize(9.5).fillColor('white').font('Helvetica');
    T('B201, Hind Saurashtra Ind.Est, Marol,', ML, 45);
    T('Andheri - Kurla Road, Andheri (E), Mumbai 400059', ML, 58);
    T('Tel: 91 22 42959123, Mail: info@kenmccoy.in, Web: www.kenmccoy.in', ML, 71);
    
    try {
        if(fs2.existsSync(KMC_LOGO)) {
            doc.image(KMC_LOGO, MR-130, 8, {fit:[130, 42]});
            doc.fontSize(16).fillColor(YEL).font('Helvetica-Bold'); 
            T('TAX INVOICE', MR-130, 65, { width: 130, align: 'center' });
        } else {
            doc.fontSize(18).fillColor(YEL).font('Helvetica-Bold'); 
            T('TAX INVOICE', MR-150, 65);
        }
    } catch(e) {}
    y=hH;

    // ──────── CUSTOMER + METADATA ────────
    const sY=y+2, cW=315, mW=CW-cW; 
    const cX=ML, mX=ML+cW;
    const mRH=18, mTH=mRH*8;
    
    doc.rect(ML, sY, CW, mTH).strokeColor('#000000').lineWidth(0.5).stroke();
    doc.moveTo(mX, sY).lineTo(mX, sY+mTH).strokeColor('#000000').lineWidth(0.5).stroke();
    ...
    let cf=sY+22;
    cd.forEach(([l,v])=>{
        doc.fontSize(9).fillColor(BLK).font('Helvetica-Bold'); T(l,cX+10,cf,{width:70});
        doc.font('Helvetica'); 
        doc.text(v, cX+85, cf, {width: cW-95, lineBreak: true});
        const th = doc.heightOfString(v, {width: cW-95, font: 'Helvetica', size: 9});
        cf += Math.max(mRH, th + 2); 
    });
    ...
    md.forEach(([l,v,o])=>{
        ...
        doc.fontSize(8.5).fillColor(o?ORANGE:GRY).font('Helvetica-Bold'); T(l,mX+5,mf+5,{width:mLW-10,align:'right'});
        doc.moveTo(mX+mLW,mf).lineTo(mX+mLW,mf+mRH).strokeColor('#aaaaaa').lineWidth(0.5).stroke();
        doc.fontSize(9).fillColor(o?ORANGE:BLK).font('Helvetica-Bold'); T(v,mX+mLW+5,mf+5,{width:mW-mLW-10,align:'center'});
        mf+=mRH;
    });
    ...
    const chgH = Math.max(20, doc.heightOfString(chgText, {width: sw[1]-10, font: 'Helvetica-Bold', size: 9.5}) + 10);
    ...
    let tableInnerTop = y-chgH-22;

    (invoice.candidates||[]).forEach((c,idx)=>{
        let rY = y;
        
        doc.fontSize(10).fillColor(BLK).font('Helvetica');
        ...
        if(c.designation) {
            doc.fontSize(9.5).font('Helvetica-Bold'); doc.text('Designation:', sx[1]+15, rY, {width:80, lineBreak: false});
            doc.font('Helvetica'); doc.text(txt(c.designation), sx[1]+95, rY, {width: sw[1]-100});
            rY += Math.max(16, doc.heightOfString(txt(c.designation), {width: sw[1]-100}) + 4);
        }
        if(c.level) {
            doc.fontSize(9.5).font('Helvetica-Bold'); doc.text('Level:', sx[1]+15, rY, {width:80, lineBreak: false});
            doc.font('Helvetica'); doc.text(txt(c.level), sx[1]+95, rY, {width: sw[1]-100});
            rY += Math.max(16, doc.heightOfString(txt(c.level), {width: sw[1]-100}) + 4);
        }
        
        rY += 4; 
        
        doc.dash(1, {space: 1.5}).strokeColor('#000000').lineWidth(0.5);
        doc.moveTo(ML, rY).lineTo(MR, rY).stroke();
        doc.undash();
        
        let valY = y + ((rY-y)/2) - 6; 
        if(idx===0){
            doc.fontSize(10).font('Helvetica-Bold').fillColor(BLK);
            // Change 3: Fix '1' prefix by using 'Rs. ' instead of symbol
    ...
    // Change 1: Add a horizontal line that effectively ends the candidate table if empty space follows
    doc.dash(1, {space: 1.5}).strokeColor('#000000').lineWidth(0.5);
    doc.moveTo(ML, y).lineTo(MR, y).stroke();
    doc.undash();

    const sTableBot=y; 
    doc.rect(ML,sTableTop,CW,sTableBot-sTableTop).strokeColor('#000000').lineWidth(0.5).stroke();
    
    doc.dash(1, {space: 1.5}).strokeColor('#000000').lineWidth(0.5);
    sx.slice(1, -1).forEach(x=>{doc.moveTo(x,tableInnerTop).lineTo(x,sTableBot).stroke();});
    doc.undash();

    // ──────── GST & TOTAL SECTION ────────
    // Change 2: labels was coming out of box. Let labels start from sx[2] (Salary column) 
    // to give them more width (sw[2] + sw[3]).
    const gL = sx[2], gW = sw[2] + sw[3], gVL = sx[4], gVW = sw[4];
    const gstH = 22;

    const dR_T=(l,v,b, dts=true)=>{
        doc.dash(1, {space: 1.5}).strokeColor('#000000').lineWidth(0.5);
        doc.moveTo(gL,y).lineTo(MR,y).stroke();
        doc.moveTo(gVL,y).lineTo(gVL,y+gstH).stroke();
        doc.undash();
        
        doc.fontSize(9.5).fillColor(BLK).font(b?'Helvetica-Bold':'Helvetica'); 
        T(l,gL+2,y+6,{width:gW-4,align:'right'});
    ...
    doc.rect(gL,gstTop,gW+gVW,y-gstTop).strokeColor('#000000').lineWidth(0.5).stroke();
    doc.moveTo(gL,gstTop).lineTo(gL,y).stroke(); // left edge for box

    // ──────── AMT IN WORDS ────────
    // Align right, but below the GST box as requested in red line
    y+=10;
    const ws=numberToWords(invoice.netPayable)+' Only';
    doc.fontSize(10.5).fillColor(BLK).font('Helvetica-Bold');
    const al='Amt In Words: ', af=al+ws;
    T(al,MR-doc.widthOfString(af),y); doc.fillColor(ORANGE); T(ws,MR-doc.widthOfString(ws),y);
    y+=20;

    // ──────── BANK + SIGNATURE ────────
    const bW2=315, sW2=CW-bW2, bY=y, bR2=22;
    const sigBoxH = 170; 
    ...
    doc.rect(ML, bY, CW, sigBoxH).strokeColor('#000000').lineWidth(0.5).stroke();
    doc.moveTo(sigX, bY).lineTo(sigX, bY+sigBoxH).strokeColor('#000000').lineWidth(0.5).stroke();
    ...
    bD.forEach(([l1,v1,l2,v2])=>{
        ...
        doc.fontSize(8.5).fillColor(GRY).font('Helvetica-Bold'); T(l1,ML+5,sigRowsY+6,{width:70});
        ...
        doc.fontSize(8.5).fillColor(GRY).font('Helvetica-Bold'); T(l2,ML+bkL_Col+5,sigRowsY+6,{width:70});
        doc.fillColor(BLK).font('Helvetica');
        const v2Size = (v2.length > 28) ? 7.5 : 9;
        doc.fontSize(v2Size).text(v2, ML+bkL_Col+75, sigRowsY+5, {width: bkR_Col-80, lineBreak: true, height: bR2-2});
        sigRowsY+=bR2;
    });
    ...
    if(sigImg) { 
        try { 
            ...
            doc.image(Buffer.from(b6,'base64'), sigX + (sW2-130)/2, bY+50, {width:130}); 
        } catch(e) {} 
    }
    if(sealImg) { 
        try { 
            ...
            doc.image(sb, sigX + sW2 - 80, bY+55, {width:75}); 
        } catch(e) {} 
    }
    ...
    doc.rect(ML,y+tcHdr,CW,tcBody).strokeColor('#000000').lineWidth(0.5).stroke();
    ...
    tms.forEach((t,i)=>T(t,ML+15,y+tcHdr+8+(i*12)));
    ...
    console.log("Successfully replaced the PDF Generation block with fixes for '1' prefix, box expansion, and dotted lines!");
} else {
