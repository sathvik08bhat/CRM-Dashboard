...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    (invoice.candidates||[]).forEach((c,idx)=>{
        ...
        if(idx===0){
            ...
            T('\u20B9 '+fmt(invoice.chargeableSalary),sx[2],valY,{width:sw[2]-10,align:'right'});
            T((invoice.rate||0).toFixed(2)+'%',sx[3],valY,{width:sw[3],align:'center'});
            T('\u20B9 ' + fmt(invoice.chargeableAmount), sx[4], valY, {width:sw[4]-10,align:'right'});
        }
        y = rY;
    });
    ...
    // EXTENDED BOX FOR GST TO ALIGN TO BOTTOM OF DESCRIPTION COLUMN
    const gstH = 20;
    const dR_T=(l,v,b, dts=true)=>{
        ...
        doc.fontSize(9).fillColor(BLK).font(b?'Helvetica-Bold':'Helvetica'); 
        T(l,gL+2,y+6,{width:gW-4,align:'right'});
        
        doc.fontSize(10).fillColor(BLK).font(b?'Helvetica-Bold':'Helvetica'); 
        T('\u20B9 '+fmt(v),gVL,y+5,{width:gVW-10,align:'right'});
        y+=gstH;
    };
    ...
    doc.moveTo(gL,gstTop).lineTo(gL,y).stroke(); // left solid for box
    ...
    console.log("Successfully replaced the PDF Generation block with ULTRA PRECISION!");
} else {
